...
public class Eight extends Activity implements OnClickListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.getclass_8);
		goToNew();
		getIntentt();
	declaration();
	filter();
	...
	public void goToNew(){
		AlertDialog alertDialog = new AlertDialog.Builder(Eight.this).create();
...
else if(mon8.getText().toString().contains(" ")){
		...
		if (v == check) {
			...
			os = new OutputStreamWriter(openFileOutput(
				...
				if(maxDays==6){
					...
					os.close();
					Intent intent = new Intent(Eight.this, Calender.class);
					intent.putExtra("maxDays",maxDays);
					intent.putExtra("variety",8);
